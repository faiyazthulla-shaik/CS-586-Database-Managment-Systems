...
def create_WDICountry(db_conn):
	cur=db_conn.cursor()
	create_stmt="CREATE TABLE WDI_Country("\
...
def create_WDISeries(db_conn):
    cur = db_conn.cursor()
    create_stmt = "CREATE TABLE WDI_Series(" \
...
def create_WDICountry_Series(db_conn):
    cur = db_conn.cursor()
    create_stmt = "CREATE TABLE WDI_Country_Series(" \
                  "  CountryCode CHAR(3) NOT NULL," \
                  "  SeriesCode VARCHAR NOT NULL," \
...
def create_WDIData(db_conn):
    cur = db_conn.cursor()
    create_stmt = "CREATE TABLE WDI_Data(" \
...
def create_WDIFootNote(db_conn):
    cur = db_conn.cursor()
    create_stmt = "CREATE TABLE WDI_FootNote(" \
                  "  CountryCode CHAR(3) NOT NULL," \
                  "  SeriesCode VARCHAR NOT NULL," \
...
def create_WDISeries_Time(db_conn):
    cur = db_conn.cursor()
    create_stmt = "CREATE TABLE WDI_Series_Time(" \
                  "  SeriesCode VARCHAR NOT NULL," \
			"  Year VARCHAR NOT NULL,"\
                  "  Description TEXT NOT NULL," \
			"  FOREIGN KEY (SeriesCode)REFERENCES WDISeries(SeriesCode));"
